...
export async function POST(req: Request) {
  try {
    ...
    // =========================
    // 1. EMAIL TO HOSPITAL (critical — must succeed for the request to count as successful)
    // =========================
    let hospitalEmailOk = false
    try {
      ...
      if (hospitalResult.error) {
        // Resend returned a structured error instead of throwing — surface it, don't silently pass
        console.error("❌ Hospital email error:", hospitalResult.error)
      } else {
        hospitalEmailOk = true
      }
    } catch (err) {
      console.error("❌ Hospital email threw:", err)
    }

    // =========================
    // 2. THANK YOU EMAIL TO PATIENT (nice-to-have — must NEVER block the response)
    // =========================
    try {
      ...
      if (patientResult.error) {
        console.error("⚠️ Patient confirmation email error (non-fatal):", patientResult.error)
      } else {
        console.log("📤 Patient confirmation email sent")
      }
    } catch (err) {
      // Swallow: the patient email is a courtesy, not a requirement for a successful submission
      console.error("⚠️ Patient confirmation email threw (non-fatal):", err)
    }

    // The submission is only a failure if the hospital never found out about the lead
    if (!hospitalEmailOk) {
      return NextResponse.json(
        { success: false, error: "Could not notify the clinic. Please try again or call us directly." },
        { status: 502 }
      )
    }
